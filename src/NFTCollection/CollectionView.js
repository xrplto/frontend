...
const TradersTab = React.memo(({ slug }) => {
  ...
  useEffect(() => {
    ...
    axios.get(`${BASE_URL}/nft/analytics/collection/${slug}/traders?limit=50&sortBy=${sortBy}&interval=${interval}${searchParam}`)
            ...
            <button
              ...
              className={cn(
                ...
                sortBy === s.key
                  ...
                  : isDark ? 'text-white/50 hover:text-white/80 hover:bg-white/5' : 'text-gray-400 hover:text-gray-600 hover:bg-gray-50'
              )}
            >
              {s.label}
            </button>
        ...
        <table className="w-full">
          <thead>
            <tr className={cn('border-b', isDark ? 'border-white/5' : 'border-gray-100')}>
              ...
              <th className="py-2 px-2 text-right text-[10px] font-medium uppercase tracking-wider text-green-500/70">Bought</th>
              <th className="py-2 px-2 text-right text-[10px] font-medium uppercase tracking-wider text-red-500/70">Sold</th>
          ...
          <tbody>
            {traders.map((t, idx) => {
              ...
              const isTop = idx < 3;
              return (
                <tr key={addr} className={cn('border-b', isDark ? 'border-white/5 hover:bg-white/[0.02]' : 'border-gray-100 hover:bg-gray-50')}>
                  <td className="py-2.5 pr-2">
                    <span className={cn('text-[11px] font-medium', isTop ? idx === 0 ? 'text-yellow-500' : idx === 1 ? 'text-gray-400' : 'text-amber-600' : isDark ? 'text-white/30' : 'text-gray-400')}>{idx + 1}</span>
                  </td>
                  <td className="py-2.5 px-2">
                    <Link href={`/address/${addr}`} target="_blank" className={cn('text-[12px] font-mono hover:text-primary transition-colors', isDark ? 'text-white/80' : 'text-gray-700')}>
