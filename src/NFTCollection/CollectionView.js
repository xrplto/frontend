...
const GRID_OPTIONS = [
  {
    value: 10,
    label: 'Large',
    cols: 'grid-cols-2 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 xl:grid-cols-10'
  },
  {
    value: 12,
    label: 'Medium',
    cols: 'grid-cols-3 sm:grid-cols-5 md:grid-cols-7 lg:grid-cols-9 xl:grid-cols-12'
  },
  {
    value: 18,
    label: 'Small',
    cols: 'grid-cols-4 sm:grid-cols-6 md:grid-cols-9 lg:grid-cols-12 xl:[grid-template-columns:repeat(18,minmax(0,1fr))]'
  }
];
...
function Label({ color = 'default', variant = 'ghost', children, className = '' }) {
  const colorStyles = {
    default:
      variant === 'ghost' ? 'bg-gray-500/10 text-gray-500' : 'border-gray-500/20 text-gray-500',
    primary: variant === 'ghost' ? 'bg-primary/10 text-primary' : 'border-primary/20 text-primary',
    success:
      variant === 'ghost' ? 'bg-green-500/10 text-green-500' : 'border-green-500/20 text-green-500',
    warning:
      variant === 'ghost'
...
function AttributeFilter({ attrs, setFilterAttrs, activeFilters = [] }) {
  ...
  const handleAttrChange = (title, key) => {
    ...
    if (found) {
      found.value = found.value.includes(key)
        ? found.value.filter((v) => v !== key)
        : [...found.value, key];
          ...
          <div className="flex flex-wrap gap-1">
            {(() => {
              ...
              const sortedKeys = Object.keys(items).sort(
                (a, b) => (items[b].count || items[b]) - (items[a].count || items[a])
              );

              return sortedKeys.map((key) => {
                const data = items[key];
                const itemCount = data.count || data;
                const isChecked = attrFilter
                  .find((elem) => elem.trait_type === expandedTrait)
                  ?.value?.includes(key);
  ...
  ({ collection, isDark }) => {
    ...
    useEffect(() => {
      ...
      if (activeTraits.length > 0) {
        query.traits = activeTraits
          .flatMap((a) => a.value.map((v) => `${a.trait_type}:${v}`))
          .join(',');
      }
    ...
    useEffect(() => {
      const handleClickOutside = (e) => {
        ...
        if (listedDropdownRef.current && !listedDropdownRef.current.contains(e.target))
    ...
    useEffect(() => {
      ...
      if (activeTraits.length > 0) {
        params.append(
          'traits',
          activeTraits.flatMap((a) => a.value.map((v) => `${a.trait_type}:${v}`)).join(',')
        );
      }
            ...
            <div className="flex flex-wrap gap-1 mt-2">
              {filterAttrs
                ...
                .map((attr) =>
                  attr.value.map((v) => (
                    <span
                      key={`${attr.trait_type}-${v}`}
                      className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-medium bg-blue-500/10 text-blue-500"
                    >
                      {attr.trait_type}: {v}
                      <button
                        onClick={() => {
                          const updated = filterAttrs.map((a) =>
                            a.trait_type === attr.trait_type
                              ? { ...a, value: a.value.filter((val) => val !== v) }
                              : a
                          );
              ...
              <button
                onClick={() => {
                  setFilterAttrs(filterAttrs.map((a) => ({ ...a, value: [] })));
                  setSortBy('price-low');
                  setListed('');
                }}
  ...
  useEffect(() => {
    ...
    const initChart = async () => {
      try {
        ...
        chart
          .addSeries(AreaSeries, areaOpts('#3b82f6', 'rgba(59,130,246,0.4)', 'rgba(59,130,246,0)'))
          .setData(salesData);
        chart
          .addSeries(AreaSeries, areaOpts('#eab308', 'rgba(234,179,8,0.3)', 'rgba(234,179,8,0)'))
          .setData(listingsData);
        chart
          .addSeries(AreaSeries, areaOpts('#a855f7', 'rgba(168,85,247,0.3)', 'rgba(168,85,247,0)'))
          .setData(bidsData);
    ...
    initChart();
    return () => {
      if (chartRef.current?.resizeHandler)
        window.removeEventListener('resize', chartRef.current.resizeHandler);
            ...
            <>
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-1">
                {salesData.sales.map((sale, idx) => {
                  ...
                  const imgUrl = thumb
                    ? `https://s1.xrpl.to/nft/${thumb}`
                    : sale.image?.startsWith('ipfs://')
                      ? `https://ipfs.io/ipfs/${sale.image.slice(7)}`
                      : '';
                  return (
                    <div
...
const HoldersTab = React.memo(({ slug }) => {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([holdersRes, distRes]) => {
        ...
        setTotalPages(
          holdersRes.data?.pagination?.pages ||
            Math.ceil((holdersRes.data?.stats?.totalOwners || 0) / LIMIT)
        );
      })
      .catch(console.error)
      .finally(() => setLoading(false));
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      setSearchResult({
        error: err.response?.status === 404 ? 'Address not found' : 'Search failed'
      });
    } finally {
      setSearching(false);
    }
  ...
  const top5Pct =
    total > 0
      ? ((topHolders.slice(0, 5).reduce((s, h) => s + h.count, 0) / total) * 100).toFixed(1)
      : '0';
  const top10Pct =
    total > 0
      ? ((topHolders.slice(0, 10).reduce((s, h) => s + h.count, 0) / total) * 100).toFixed(1)
      : '0';
  const top20Pct =
    total > 0
      ? ((topHolders.slice(0, 20).reduce((s, h) => s + h.count, 0) / total) * 100).toFixed(1)
      : '0';
              ...
              <div key={row.label} className="flex items-center gap-2">
                ...
                >
                  <div
                    className={cn(
                      'h-full rounded-full',
                      parseFloat(row.pct) > 50 ? 'bg-orange-500' : 'bg-primary'
                    )}
                    ...
                    <div key={tier} className="flex items-center gap-2">
                      ...
                      >
                        <div
                          ...
                          style={{ width: `${Math.min(parseFloat(nftPct), 100)}%` }}
                        />
                      </div>
                      <span
                        className={cn(
                  ...
                  <div>
                    ...
                    {searchResult.totalCollections > 0 && (
                      ...
                      >
                        ...
                        {searchResult.totalCollections > 1 ? 's' : ''})
                      </span>
                    )}
                  </div>
                  <button
...
export default function CollectionView({ collection }) {
  ...
  useEffect(() => {
    if (collectionData?.slug && collectionData?.name) {
      ...
      const normalizedName =
        typeof collectionData.name === 'object' && collectionData.name !== null
  ...
  const statsData = [
    {
      label: 'Floor',
      value: fNumber(typeof floor === 'number' ? floor : floor?.amount || 0),
      prefix: '✕',
      color: 'text-green-500'
    },
    floor7dPercent !== undefined &&
      floor7dPercent !== 0 && {
        label: '7d',
    ...
    topOfferAmount > 0 && {
      label: 'Top Offer',
      value: fNumber(topOfferAmount),
      prefix: '✕',
      color: 'text-emerald-500'
    },
    marketcap > 0 && {
      label: 'Mkt Cap',
      value: fVolume(marketcap),
      prefix: '✕',
