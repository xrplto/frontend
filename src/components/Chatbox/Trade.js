...
const Trade = ({ open, onClose, tradePartner }) => {
  ...
  const [loggedInUserOffers, setLoggedInUserOffers] = useState([
    { currency: 'XRP', amount: 0, token_type: 'token' }
  ]);
  ...
  const fetchBalances = async () => {
    ...
    try {
      ...
      const loggedInUserTotalBalance =
        Number(loggedInUserInfo.result.account_data.Balance) / 1000000;
  ...
  const processLines = (lines, account) => {
    return lines.map((line) => {
      ...
      const balance =
        account === line.account ? Math.abs(Number(line.balance)) : Number(line.balance);
  ...
  const handleLoggedInUserAssetSelect = (nft) => {
    setSelectedLoggedInUserAssets((prev) => {
      ...
      if (loggedInUserXrpBalance < requiredReserve) {
        setNftWarning(
          `Warning: You need at least ${requiredReserve} XRP in reserve to trade ${
            newSelection.length
          } NFT${
            newSelection.length > 1 ? 's' : ''
          }. Your current available balance is ${loggedInUserXrpBalance.toFixed(6)} XRP.`
        );
      } else {
        setNftWarning('');
      }
    ...
    const updateOffers = async (offers) => {
      ...
      if (field === 'amount' && isLoggedInUser) {
        ...
        if (value === '') {
          ...
          if (offer.currency === 'XRP') {
            ...
            const token = loggedInUserTokens.find(
              (t) => t.currency === offer.currency && t.issuer === offer.issuer
            );
      ...
      if (!isLoggedInUser && field === 'currency') {
        if (value === 'XRP') {
          ...
          const trustlineExists = loggedInUserLines.some(
            (line) => line.currency === value && line.account === _offers[index].issuer
          );
          ...
          if (!trustlineExists) {
            ...
            if (availableXRP < requiredXRP) {
              setTrustlineWarnings((prev) => ({
                ...prev,
                [index]: `Warning: Insufficient XRP balance to set up a trustline for ${value}. You need at least ${requiredXRP} XRP, but only have ${availableXRP.toFixed(
                  2
                )} XRP available.`
              }));
            } else {
      ...
      .then(async (res) => {
        ...
        if (ret) {
          ...
          const trustlineStatus = await trustlines.find((trustline) => {
            return (
              (trustline.LowLimit.issuer === wallet_address || trustline.HighLimit.issuer) &&
  ...
  const handleTrade = async () => {
    ...
    loggedInUserOffers.forEach((tokenInfo, index) => {
      ...
      if (
        tokenInfo.currency !== 'XRP' &&
        tokenInfo.token_type !== 'NFT' &&
        tokenInfo.amount === 0
      ) {
        console.log(
          `Invalid token amount for ${normalizeCurrencyCodeXummImpl(tokenInfo.currency)}`
        );
        showNotification(
          `Invalid token amount for ${normalizeCurrencyCodeXummImpl(tokenInfo.currency)}`,
          'error'
    ...
    try {
      ...
      let itemsRequested = partnerOffers.filter(
        (offer) => offer.token_type === 'NFT' || (offer.token_type === 'token' && offer.amount > 0)
      );
      ...
      for (const offer of itemsRequested) {
        if (offer.currency !== 'XRP' && offer.token_type !== 'NFT') {
          const trustlineExists = loggedInUserLines.some(
            (line) => line.currency === offer.currency && line.account === offer.issuer
          );
          ...
          if (!trustlineExists) {
            ...
            if (loggedInUserXrpBalance < totalRequiredXRP) {
              ...
              showNotification(
                `Insufficient XRP balance to set up trustline for ${offer.currency}`,
                'error'
              );
              return;
            }
          ...
          if (offer.token_type === 'NFT') {
            ...
            return {
              ...
              Memos: configureMemos(
                ...
                '',
                `https://xrpnft.com/nft/${offer.token_address}`
              )
            };
          } else {
      ...
      switch (wallet_type) {
        ...
        case 'crossmark':
          ...
          try {
            const { response } = await sdk.methods.bulkSignAndSubmitAndWait(paymentTxData, {
              isModifiable: false
            });
  ...
  const fetchAllowedTokens = async () => {
    try {
      const response = await axios.get(
        `${BASE_URL}/account/lines/rKxpqFqHWFWRzBuSkjZGHg9HXUYMGn6zbk?page=0&limit=100`
      );
  ...
  const isTokenAllowed = (currency, issuer) => {
    return allowedTokens.some((token) => token.currency === currency && token.issuer === issuer);
  };
        ...
        <Box key={index} display="flex" flexDirection="column" mb={2}>
          <Box display="flex" alignItems="center">
            ...
            >
              ...
              {tokens
                .filter((token) => isTokenAllowed(token.currency, token.issuer))
                .map((token) => (
  ...
  const isTradeValid = () => {
    const loggedInUserHasItem =
      loggedInUserOffers.some((offer) => offer.amount !== undefined) ||
      selectedLoggedInUserAssets.length > 0;
    const partnerHasItem =
      partnerOffers.some((offer) => offer.amount !== undefined) || selectedPartnerAssets.length > 0;
      ...
      <StyledDialogContent>
        <Grid container spacing={4}>
            ...
            <StyledPaper elevation={3}>
              ...
              <Typography variant="body1" color="text.secondary" mb={2}>
                ...
                <Box component="span" fontWeight="bold">
                  {loggedInUserXrpBalance.toFixed(6)} XRP
                </Box>
              </Typography>
          ...
          <Grid item xs={6}>
            <StyledPaper elevation={3}>
              <Typography variant="h6" fontWeight="bold" gutterBottom>
                {tradePartner.username}'s Portfolio
              </Typography>
              <Typography variant="body1" color="text.secondary" mb={2}>
                Available XRP:{' '}
                <Box component="span" fontWeight="bold">
                  {partnerXrpBalance.toFixed(6)} XRP
                </Box>
              </Typography>
              <Typography variant="subtitle1" fontWeight="bold" gutterBottom>
                Tokens to Request:
