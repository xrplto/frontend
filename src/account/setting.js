...
export default function EditProfile() {
  ...
  const logoImageUrl = accountProfile?.logo
    ? `https://s1.xrpl.to/profile/${accountProfile.logo}`
    : null;
  const bannerImageUrl = accountProfile?.banner
    ? `https://s1.xrpl.to/profile/${accountProfile.banner}`
    : null;
  ...
  const checkChanged = () => {
    ...
    else if (fileUrl1 !== logoImageUrl) return true;

    if (file2) return true;
    else if (fileUrl2 !== bannerImageUrl) return true;
  ...
  const onEditProfile = async () => {
    ...
    try {
      ...
      if (fileFlag[0] === FILE_NEW) formdata.append('imgProfile', file1);
      if (fileFlag[1] === FILE_NEW) formdata.append('imgProfile', file2);
  ...
  return (
    <>
      <Stack spacing={1} sx={{ mt: 4, mb: 3 }}>
        ...
        <CardWrapperCircle>
          ...
          >
            ...
            <img
              ...
              style={
                fileUrl1
                  ? { objectFit: 'cover', width: '100%', height: '100%', overflow: 'hidden' }
                  : { display: 'none' }
              }
            />
            <ImageIcon
        ...
        <Typography variant="p3">
          This image will appear at the top of your account page. Avoid including too much text in
          this banner image, as the dimensions change on different devices. 1400 x 350
      ...
      <Stack spacing={2} mb={3}>
        ...
        <Typography variant="p3">Only {description.length} of 1000 characters allowed.</Typography>
        <TextField
          placeholder=""
          margin="dense"
          multiline
