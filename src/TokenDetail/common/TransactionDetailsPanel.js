...
const TransactionDetailsPanel = memo(({ 
  ...
  const bestAsk = useMemo(() => (mode === 'orderbook' && asks && asks[0] ? Number(asks[0].price) : null), [mode, asks]);
  const bestBid = useMemo(() => (mode === 'orderbook' && bids && bids[0] ? Number(bids[0].price) : null), [mode, bids]);
  ...
  return (
    <Drawer
      ...
      PaperProps={{
        sx: {
          width: mode === 'orderbook' ? { md: 280, lg: 320, xl: 360 } : { md: 240, lg: 256, xl: 272 },
          minWidth: { md: 236 },
    ...
    >
      ...
      >
        ...
        {mode === 'orderbook' && pair && (
          <>
            <Typography 
              variant="caption" 
              sx={{ color: alpha(theme.palette.text.secondary, 0.7), fontSize: '0.7rem', display: 'block', mt: 0.25 }}
            >
            ...
            <Stack direction="row" spacing={1} alignItems="center" sx={{ mt: 1 }}>
              <Tooltip title="Highest price buyers are willing to pay">
                <Chip size="small" color="success" variant="outlined" label={bestBid != null ? `Best Bid ${fNumber(bestBid)}` : 'No bids'} sx={{ height: 22, fontSize: '0.7rem' }} />
              </Tooltip>
              <Tooltip title="Lowest price sellers are willing to accept">
                <Chip size="small" color="error" variant="outlined" label={bestAsk != null ? `Best Ask ${fNumber(bestAsk)}` : 'No asks'} sx={{ height: 22, fontSize: '0.7rem' }} />
              </Tooltip>
      ...
      <Box sx={{ p: 1.5, flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column' }}>
        ...
        {mode === 'orderbook' && (
          <Box sx={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
            ...
            <Box sx={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
              <Box sx={{ p: 1, mx: 0.5, borderRadius: '8px', background: alpha(theme.palette.error.main, 0.06) }}>
                <Stack direction="row" alignItems="center" spacing={0.5}>
            ...
            {spread.spreadAmount > 0 ? (
              <Box sx={{ px: 1, py: 0.75, mx: 0.5, background: alpha(theme.palette.warning.main, 0.06), borderTop: `1px dashed ${alpha(theme.palette.warning.main, 0.3)}`, borderBottom: `1px dashed ${alpha(theme.palette.warning.main, 0.3)}` }}>
              ...
              <Box sx={{ height: '1px', background: `linear-gradient(90deg, transparent 0%, ${alpha(theme.palette.divider, 0.3)} 50%, transparent 100%)`, m: 1 }} />
            )}

            {/* Bottom half: Bids */}
            <Box sx={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
              <Box sx={{ p: 1, mx: 0.5, borderRadius: '8px', background: alpha(theme.palette.success.main, 0.06) }}>
                <Stack direction="row" alignItems="center" spacing={0.5}>
            ...
            {bids.length === 0 && asks.length === 0 && (
              <Box sx={{ py: 3, textAlign: 'center', borderRadius: '12px', background: alpha(theme.palette.background.paper, 0.1), border: `1px solid ${alpha(theme.palette.divider, 0.08)}`, m: 1.5 }}>
                <Typography variant="body2" sx={{ color: alpha(theme.palette.text.secondary, 0.8), fontSize: '0.85rem', fontWeight: 500 }}>
                  No orders available
                </Typography>
              </Box>
            )}
