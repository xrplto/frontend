...
const PairsSelect = memo(({ token, pair, setPair }) => {
  ...
  const getPairs = useCallback(() => {
    ...
    setIsLoading(true);
    axios
      .get(`${process.env.API_URL}/pairs?md5=${token.md5}`)
      .then((res) => {
      ...
      pairs.map(({ id, pair, curr1, curr2 }) => (
        <MenuItem key={id} value={pair} sx={{ py: 0.5 }}>
          <Stack direction="row" alignItems="center" spacing={0.5}>
            ...
            <Typography
              variant="caption"
              sx={{ color: darkMode ? '#007B55' : '#5569ff', fontWeight: 600 }}
            >
              {curr2.name}
            </Typography>
            {!isMobile && (
  ...
  if (!pair)
    return (
      <FormControl
        sx={{ m: { xs: 0.5, sm: 1 }, minWidth: isMobile ? 200 : 300, width: '100%' }}
        size="small"
      >
    ...
    >
      ...
      <Select
        ...
        sx={{
          ...
          width: '100%',
          '& .MuiSelect-select': { py: isMobile ? 0.75 : 1 }
        }}
