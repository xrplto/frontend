...
export default function OrderBook({ pair, asks, bids, onAskClick, onBidClick }) {
  ...
  const buildPriceLevels = (levels, orderType = ORDER_TYPE_BIDS) => {
    return levels.slice(0, 30).map((level, idx) => {
      ...
      let value = level.value.toFixed(2); // fNumber(level.value);
      let sumAmount = level.sumAmount; // fNumber(level.sumAmount);
      let sumValue = level.sumValue; // fNumber(level.sumValue);
      ...
      if (isNew) bidBackgroundColor = `#00AB5588`;
      else
      ...
      if (isNew) askBackgroundColor = `#FF484288`;
      else
        askBackgroundColor = `linear-gradient(to left, #FF484233, rgba(0, 0, 0, 0.0) ${depth}%, rgba(0, 0, 0, 0.0))`;
      // askBackgroundColor = `#FF4842${depth}`;

      if (idx < selected[0]) bidBackgroundColor = `#00AB5588`;

      if (idx < selected[1]) askBackgroundColor = `#FF484288`;
          ...
          {isBid ? (
            ...
            >
              ...
              <TableCell
                sx={{ p: 0, pr: 1 }}
                align="right"
                style={{ color: `${isNew || selected[0] > idx ? '' : '#118860'}` }}
              >
            ...
            >
              ...
              <TableCell
                sx={{ p: 0, pl: 1 }}
                style={{ color: `${isNew || selected[1] > idx ? '' : '#bb3336'}` }}
              >
                <NumberTooltip number={price} pos="bottom" />
              </TableCell>
    ...
    return (
      <Stack>
        <Grid container spacing={0} sx={{ p: 0 }}>
          ...
          <Grid item xs={12} md={6} lg={6} sx={{ p: 0 }}>
            <Typography
              align="right"
              variant="subtitle1"
              sx={{ color: '#B72136', ml: 2, mt: 2, mb: 1 }}
            >
        ...
        {bids.length === 0 && asks.length === 0 && (
          <LoaderContainer>{/* <PuffLoader color={"#00AB55"} size={50} /> */}</LoaderContainer>
        )}
      </Stack>
    );
  } else return 'HIDDEN PAGE.';
}
