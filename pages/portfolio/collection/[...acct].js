...
export default function CollectedCreatedNFTs({ type, account, limit, collection }) {
  ...
  useEffect(() => {
    // seems like useless, so created resetNfts() to be used in like handleChangeSearch()
    ...
    <Grid container spacing={1}>
      {nfts.map((nft) => (
        <Grid
          ...
          lg={2.4}
          xl={1.5}
          key={nft.uuid || nft.id || nft.tokenId || `nft-${nft.issuer}-${nft.taxon}`}
        >
          {collection ? (
            <NFTCard nft={nft} />
          ) : (
        ...
        {collection && (
          <Box display="flex" justifyContent="start" mt={4}>
            <IconButton onClick={handleBack}>
              ...
              <Typography variant="s3" fontSize="medium">
                Go back
              </Typography>
            </IconButton>
          </Box>
