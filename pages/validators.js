...
function ValidatorsPage({
  ...
  const getValidatorPerformance = () => {
    ...
    const unlValidators = validators.filter(
      (v) =>
        v.unl &&
        Array.isArray(v.unl) &&
    ...
    return {
      ...
      worst:
        nonVoting.length > 0
          ? [...nonVoting, ...sorted.slice(-(10 - Math.min(nonVoting.length, 10))).reverse()].slice(
              0,
              10
            )
  ...
  return (
    <>
      <Head>
        ...
        <meta
          name="description"
          content={`Monitor ${validators.length} XRPL validators, ${nodes.length} nodes, and ${amendments.length} protocol amendments. Real-time voting data, UNL performance metrics, and network statistics.`}
        />
        ...
        <meta
          ...
          content={`Track ${validators.length} validators, ${nodes.length} nodes, and ${amendments.filter((a) => !a.enabled && a.count > 0).length} active amendment votes on the XRP Ledger network.`}
        />
        ...
        <meta
          ...
          content={`${validators.length} validators • ${nodes.length} nodes • ${amendments.filter((a) => !a.enabled && a.count > 0).length} active votes`}
        />
                    ...
                    <Card>
                      <CardContent>
                        ...
                        <Typography variant="body2" color="text.secondary">
                          {
                            amendments.filter(
                              (a) => !a.enabled && !a.majority && a.count >= a.threshold * 0.75
                            ).length
                          }{' '}
                          near consensus
                        </Typography>
                              ...
                              <TableBody>
                                {validatorPerformance.worst.map((v, i) => (
                                  <TableRow
                                    ...
                                    sx={{
                                      backgroundColor:
                                        v.votesCount === 0 ? 'error.light' : 'transparent'
                                    }}
                                  >
                                    <TableCell>
                                      <Box>
                              ...
                              <TableBody>
                                {(() => {
                                  ...
                                  return validators
                                    ...
                                    .map((v) => {
                                      const votesOnRecent =
                                        v.votes?.amendments?.filter((a) =>
                                          recentAmendments.includes(a)
                                        ).length || 0;
                                      return {
                                        domain:
                                          v.domain ||
                                          v.domain_legacy ||
                                          truncateKey(v.master_key, 16),
                                    ...
                                    .map((v, i) => (
                                      <TableRow
                                        ...
                                        sx={{
                                          backgroundColor:
                                            v.votesCount === 0 ? 'warning.light' : 'transparent'
                                        }}
                                      >
                                        <TableCell>
                                          <Box>
                          ...
                          <Box sx={{ maxHeight: 350, overflow: 'auto' }}>
                            {serverVersions.map((version, index) => {
                              const nodeCount = nodes.filter(
                                (n) => n.server_version === version || n.version === version
                              ).length;
                              return (
                                <Box
                                  key={index}
                            ...
                            <CardContent>
                              ...
                              {validator.server_version && (
                                <Box sx={{ mt: 2 }}>
                                  ...
                                  <Typography variant="body2">
                                    {validator.server_version.version_full ||
                                      validator.server_version.version ||
                                      'N/A'}
                                  </Typography>
                                </Box>
                              )}
                              ...
                              {validators.map((validator, index) => (
                                <TableRow key={index} hover>
                                  ...
                                  <TableCell>
                                    <Typography
                                      variant="body2"
                                      title={formatDate(validator.last_seen)}
                                    >
                                      {formatRelativeTime(validator.last_seen)}
                                    </Typography>
                                  </TableCell>
                          ...
                          <NodeCard key={index}>
                            <CardContent>
                              ...
                              >
                                ...
                                <StatusChip
                                  label={
                                    getNodeStatus(node.last_seen) === 'online'
                                      ? 'Online'
                                      : 'Offline'
                                  }
                                  size="small"
                                ...
                                {amendment.count > 0 && (
                                  <Box sx={{ mt: 2 }}>
                                    ...
                                    <LinearProgress
                                      variant="determinate"
                                      value={Math.min(
                                        (amendment.count / amendment.threshold) * 100,
                                        100
                                      )}
                                      sx={{ height: 6, borderRadius: 3 }}
                                      color={
                                        amendment.count >= amendment.threshold
                                          ? 'success'
                                          : 'primary'
                                      }
                                    />
                            ...
                            <TableBody>
                              {amendments
                                .filter((a) => {
                                  if (amendmentFilter === 'all') return true;
                                  if (amendmentFilter === 'enabled') return a.enabled;
                                  if (amendmentFilter === 'pending')
                                    return a.majority && !a.enabled;
                                  if (amendmentFilter === 'voting')
                                    return !a.enabled && !a.majority;
                                  return true;
                                })
                                .sort((a, b) => {
                                  // Sort by: enabled first, then by support percentage
                                    ...
                                    {expandedAmendment === amendment.amendment_id && (
                                      <TableRow>
                                        <TableCell colSpan={7} sx={{ py: 0 }}>
                                          <Collapse
                                            in={expandedAmendment === amendment.amendment_id}
                                            timeout="auto"
                                            unmountOnExit
                                          >
                                            <Box sx={{ p: 2, bgcolor: 'background.default' }}>
                                              ...
                                              >
                                                ...
                                                {amendmentVotes[amendment.amendment_id]?.length ||
                                                  0}{' '}
                                                of {amendment.count})
                                              </Typography>
                                              {amendmentVotes[amendment.amendment_id]?.length >
                                              0 ? (
                                                <Box>
                                                  ...
                                                  >
                                                    {amendmentVotes[amendment.amendment_id].map(
                                                      (vote, i) => (
                                                        <Chip
                                                          key={i}
                                                          label={
                                                            vote.domain ||
                                                            vote.domain_legacy ||
                                                            truncateKey(vote.validator, 12)
                                                          }
                                                          size="small"
                                                          variant={
                                                            vote.verified ? 'filled' : 'outlined'
                                                          }
                                                          color={
                                                            vote.verified ? 'success' : 'default'
                                                          }
                                                          icon={
                                                            vote.verified ? (
                                                ...
                                                <Box>
                                                  <Typography
                                                    ...
                                                    sx={{ mb: 1 }}
                                                  >
                                                    {amendment.count} validators are supporting this
                                                    amendment
                                                  </Typography>
                                                  <Typography
                                                    variant="caption"
                                                  ...
                                                  >
                                                    ...
                                                    about validator vote structure.
                                                  </Typography>
                                                  {validators.length > 0 && (
                                                    <Box sx={{ mt: 2 }}>
                                                      <Typography
                                                      ...
                                                      >
                                                        ...
                                                        {validators.filter((v) =>
                                                          ...
                                                          <Typography
                                                            ...
                                                            color="text.secondary"
                                                          >
                                                            Unable to match voting validators.
                                                            Showing count: {amendment.count}
                                                          </Typography>
                                                        )}
                                                      </Box>
                                                    </Box>
  ...
  try {
    ...
    return {
      props: {
        ...
        initialAmendments: Array.isArray(amendmentsData)
          ? amendmentsData
