...
export default function Overview({ data }) {
  ...
  const bannerImage = profile.banner
    ? `https://s1.xrpl.to/profile/${profile.banner}`
    : default_banner;
...
export async function getServerSideProps(ctx) {
  ...
  const imgUrl = banner
    ? `https://s1.xrpl.to/profile/${banner}`
    : 'https://xrpl.to/static/webp.png';
  ...
  ogp.desc = description
    ? description
    : 'Manage your XRPL account with ease: Create and modify offers, establish trustlines, and gain insights into your account activity on our user-friendly platform.';
